...
let rec sort l =
  let rec partition pivot l left right =
    match l with
    | []        -> (left, right)  (* if l = empty, return left and right *)
    (* if l is of pattern head :: tail, execute expr starting at if *)
    | c :: rest -> if (compare c pivot) < 0 
                   (* if c < pivot, call partition on same pivot, l = rest, left = left with c, right *)
                   then partition pivot rest (c :: left) right
                   (* if c >= pivot, call partition on same pivot, l = rest, left, right = right with c *)
                   else partition pivot rest left (c :: right) in
  (* THIS IS EXECUTED FIRST *)
  match l with
  | []        -> l (* if l is empty, return l *)
  | h :: []   -> l (* if l is one element, return l *)
  (* if l has more than one element *)
  (* initialization case *)
                     (* finds left, right for use in next line *)   
  | h :: rest -> let (left, right) = partition h rest [] [] in (* head is new pivot, l is rest of list, left and right are empty*)
                 (sort left) @ [h] @ (sort right);; (* THIS IS EXECUTED LAST: sort the left and right lists, concatenate around pivot *)
...
let productions gram : (string * string list) list = (* gram is of form (string * string list) list *)
  (* prods returns (lhs, rhss) *)
  let prods (lhs, rhss) = map (fun rhs -> (lhs, rhs)) rhss in 
  fold_left (@) [] (map prods gram);; (* concatenate the result of map prods gram into empty list *)
...
type ast_sl = ast_s list (* sl = statement list? *)
and ast_s =
| AST_error
| AST_assign of (string * ast_e) (* S-> id := E*)
| AST_read of string (* S-> read id *)
| AST_write of ast_e (* S-> write E *)
| AST_if of (ast_c * ast_sl) (*  S -> if C SL end TODO: remember to match end? *)
| AST_while of (ast_c * ast_sl) (*  S -> while C SL end TODO: remember to match end? *)
and ast_e =
| AST_binop of (string * ast_e * ast_e) (* ex: 1+1, 2-1, 3*3, 4/2 *)
| AST_id of string  
| AST_num of string
| AST_parenthesized of (string * ast_e * string)
and ast_c = (string * ast_e * ast_e);; (* C-> E ro E , written here as C-> ro E E *)

let rec ast_ize_P (p:parse_tree) : ast_sl =
    match p with
   | PT_nt ("P", [sl; PT_term "$$"])
        -> ast_ize_SL sl
   | _  -> raise (Failure "program didn't start with P")
...
and ast_ize_S (s:parse_tree) : ast_s =
  match s with
  | PT_nt ("S", [PT_id id; PT_term ":="; expr])
        -> AST_assign (id, (ast_ize_expr expr))
  | PT_nt ("S", [PT_term "read"; PT_id id])
        -> AST_read id
  | PT_nt ("S", [PT_term "write"; expr])
        -> AST_write (ast_ize_expr expr)
  | PT_nt ("S", [PT_term "if"; c; sl; PT_term "end"]) (* S-> if C SL end*)
...
and ast_ize_expr (e:parse_tree) : ast_e =
  ...
  | PT_nt ("E", [term; termtail])
    (* continue ast_izing the term
     pass result into ast_ize_expr_tail as lhs
     with termtail as tail *)
        -> ast_ize_expr_tail (ast_ize_expr term) termtail
  | PT_nt ("T", [factor; factortail])
        -> ast_ize_expr_tail (ast_ize_expr factor) factortail
  |PT_nt ("F", [PT_id id]) (* factor will be a PT_id,  PT_num or PT_term *)
        ->  AST_id id
  | PT_nt ("F", [PT_num num])
        ->  AST_num num   
  | PT_nt ("F", [PT_term "("; expr; PT_term ")"])  (* case left paren *)
        -> AST_parenthesized ("(", ast_ize_expr expr, ")" )          
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  match tail with
  | PT_nt ("TT", [])
       (* if empty, there is no rhs *)
        -> lhs
  | PT_nt ("TT", [PT_nt ("ao", [PT_term ao]); term; termtail])
        (* Since termtail may/may not be epsilon, we must check for more  
           addition operations by calling ast_ize_expr_tail again with:
               new lhs = ast_e resulting from calling ast_ize_expr on term
               new tail= termtail *)
        -> let rhs = (ast_ize_expr_tail (ast_ize_expr term) termtail) in
        AST_binop (ao, lhs, rhs)
  | PT_nt ("FT", [])
        (* if empty, there is no rhs *)
        -> lhs
  | PT_nt ("FT", [PT_nt ("mo", [PT_term mo]); factor; factortail])
        (* Since factortail may/may not be epsilon, we must check for more  
           multiplication operations by calling ast_ize_expr_tail again with:
               new lhs = ast_e resulting from calling ast_ize_expr on factor
               new tail= factortail *)
        -> let rhs = (ast_ize_expr_tail (ast_ize_expr factor) factortail) in
        AST_binop (mo, lhs, rhs)
  | _ -> raise (Failure "malformed parse tree in ast_ize_expr_tail")

and ast_ize_C (c:parse_tree) : ast_c =
  (*
     A condition's parse tree must be of form:
    ("C", [expr1ParseTree; operatorParseTree; expr2ParseTree])
   *)
  match c with
  | PT_nt ("C", [expr1; PT_nt ("rn", [PT_term operator]); expr2])
        -> (operator, (ast_ize_expr expr1), (ast_ize_expr expr2)) 
...
let get_int (v:value) : int =
    match v with
    | Value(num) -> num
    | _ -> raise (Failure "value was not an int");;
...
and interpret_assign (lhs:string) (rhs:ast_e) (mem:memory)
    ...
    (* TODO: add a new pair entry in memory for lhs = value from (interpret_expr rhs) 
        don't do anything to inp or outp (i think) 
        btw: i made function get_int (see above) that you can use to convert from type Value to an int*)
...
and interpret_read (id:string) (mem:memory)
    ...
    (* TODO: add a new pair entry in memory for (id, next int in input list) ( but inp is strings so u have to use int_of_string to convert it ) 
        don't do anything to outp (i think) *)
...
and interpret_write (expr:ast_e) (mem:memory)
    ...
    (* TODO: add to outp the value from interpret_expr expr 
        don't do anything to mem or inp (i think) *)
...
and interpret_if (cond:ast_c) (sl:ast_sl) (mem:memory)
    ...
    : bool * memory * string list * string list =
     ...
     (* TODO: interpret the condition, if it is true then call interpret_sl *)
...
and interpret_while (cond:ast_c) (sl:ast_sl) (mem:memory)
    ...
    (* TODO: interpret the condition, if true then call interpret_sl, then call interpret_while again tail recursively*)
...
and interpret_expr (expr:ast_e) (mem:memory) : value * memory =
   match expr with
   (*  hard coded all operators because I couldn't figure it out otherwise *)
   | AST_binop ("+", lhsExpr, rhsExpr)
      (* add the values from interpreting the expressions *)
        -> let (lhsVal, m) = interpret_expr lhsExpr mem in
           let (rhsVal, m) = interpret_expr rhsExpr mem in
          (Value(((get_int lhsVal) +( get_int rhsVal))), mem)
    | AST_binop ("-", lhsExpr, rhsExpr)
      (* add the values from interpreting the expressions *)
        -> let (lhsVal, m) = interpret_expr lhsExpr mem in
           let (rhsVal, m) = interpret_expr rhsExpr mem in
    ...
    | AST_binop ("/", lhsExpr, rhsExpr)
        -> let (lhsVal, m) = interpret_expr lhsExpr mem in
           ...
           (Value(((get_int lhsVal) / ( get_int rhsVal))), mem)      
   | AST_num (num)
        -> (Value (int_of_string num) , mem)
   | AST_id (id)
        -> let (name, value) = find (fun s -> fst s = id) mem in
            (Value (value), mem)
   | AST_parenthesized (lparen, expr, rparen)
        -> interpret_expr expr mem
   | _ -> raise (Failure "incorrect structure for expr")

and interpret_cond ((op:string), (lo:ast_e), (ro:ast_e)) (mem:memory)
    : value * memory =

    (* TODO: check if (lo op ro) holds , if so return true, if not false. 
        Note: you have to return a value which has to be an int so T/F should be 1/0 *)
